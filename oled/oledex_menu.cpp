...
void OledExMenu::init(){
    ...
    this->frame.rows = this->oled->page() * 8 / this->cspacing;
}

#define ceil_div(a, b) (((a) + (b) - 1) / (b))

void OledExMenu::setFrame(){
    ...
    if (this->focus_item != this->frame.top_item){
        while(this->frame.cur_item != NULL){
            rows += ceil_div(strlen(this->frame.cur_item->title) * this->rspacing, this->oled->col());
    ...
    while(this->frame.cur_item_ptr.y < this->oled->page() * 8){
